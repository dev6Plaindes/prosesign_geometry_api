...
from bim.utils.step_to_json import datos_to_shapely, ensamblaje_to_array, polygon_a_mesh_array, terreno_a_mesh_array
...
data_primaria = [
    ...
    if row["Pabellon"] == "Izquierda"
]
largos_primaria = largos_for_piso(data_primaria, largo_cuadrante)
sum_largos_primaria = sum(largos_primaria[0])
desplazamiento_y_inf = calcular_desplazamiento_y(ancho_inferior, e_muro, ancho_cuadrante, borde="inferior")
...
for index, largos_prim in enumerate(largos_primaria):
  nivel = index + 1
  create_structure(ensamblaje, largos_prim, ancho_inferior, pos_centro_primaria, desplazamiento_y_inf, "Inferior", posicion_puerta="top", nivel=index + 1, largo_bloque_fijo=sum_largos_primaria, max_nivel=max_nivel_primaria)
  generate_techo(
        ensamblaje,
        largos_habitaciones=largos_prim,
...
data_secundaria = [
    row
    for row in data_dict_ambientes
    if row["Pabellon"] == "Derecha"
]
largos_secundaria = largos_for_piso(data_secundaria, largo_cuadrante)
desplazamiento_y_sup = calcular_desplazamiento_y(ancho_superior, e_muro, ancho_cuadrante, borde="superior")
# desplazamiento_x_sec = ((sum(largos_secundaria[0]) - ancho_cuadrante) / 2) * -1
sum_largos_sec = sum(largos_secundaria[0])
...
for index, largos_sec in enumerate(largos_secundaria):
  nivel = index + 1
  create_structure(ensamblaje, largos_sec, ancho_inferior, pos_centro_secundaria, desplazamiento_y_sup, "Secundaria", posicion_puerta="bottom", nivel=index + 1, largo_bloque_fijo=sum(largos_sec), max_nivel=max_nivel_sec)
  generate_techo(
        ensamblaje,
...
if not data_2do_cuadrante_builded_verif:
    ...
    largos_inicial = largos_for_piso(data_inicial, ancho_sobrante)
    desplazamiento_y_sup = pos_centro[0]
    pos_centro_inicial = calcular_rango_centrado(pos_centro, sum(largos_inicial[0]))[0]
...
largos_admin = largos_for_piso(data_admin, ancho_sobrante)
pos_centro_admin = calcular_rango_centrado(pos_centro, sum(largos_admin[0]))

max_nivel_admin = len(largos_admin)
for index, largo_admin in enumerate(largos_admin):
  sum_largo_admin = sum(largo_admin)
  nivel = index + 1
  ...
  generate_techo(
        ensamblaje,
        largos_habitaciones=largo_admin,
        ancho_hab=ancho_hab,
