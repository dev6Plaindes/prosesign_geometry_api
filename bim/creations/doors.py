...
def _generate_door_geometry(
    ...
    desfase_z: float,
    pos_columnas: list,
    desplazamiento_x: float  # <--- Agregamos este parámetro para tener el origen real absoluto
):
    ...
    if posicion in ["bottom", "top"]:
        ...
        for col_inicio, col_fin in pos_columnas:
            ...
            if max(puerta_izq_global, zona_col_izq) < min(puerta_der_global, zona_col_der):
                # Desplazamos el inicio de la puerta inmediatamente a la derecha de la columna
        ...
        if (p_centro_x + (ancho_p / 2)) > limite_derecho_hab:
            # Fallback dinámico: Pegamos la puerta al inicio izquierdo del muro de esta habitación (borde_x)
            p_centro_x = borde_x + (ancho_p / 2)

            # Repetimos un chequeo rápido para asegurarnos que este retorno no colisione con otra columna
            puerta_izq_global = p_centro_x - (ancho_p / 2)
            for col_inicio, col_fin in pos_columnas:
                col_inicio_global = col_inicio + desplazamiento_x
                col_fin_global = col_fin + desplazamiento_x
                ...
                if max(puerta_izq_global, col_inicio_global - holgura) < min(puerta_izq_global + ancho_p, col_fin_global + holgura):
                    # Si choca en el extremo izquierdo, la empujamos a la derecha de esa columna en específico
                    p_centro_x = col_fin_global + holgura + (ancho_p / 2)
                    break
