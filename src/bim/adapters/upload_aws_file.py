...
def subir_archivo_a_s3(archivo_binario, nombre_archivo: str, bucket_name: str) -> str:
    ...
    aws_region = os.getenv("AWS_DEFAULT_REGION", "us-east-1") # Región por defecto si no se especifica
    ...
    try:
        ...
        # Guardar una copia en bytes para evitar problemas de streams cerrados por boto3
        file_bytes = None
        if hasattr(archivo_binario, "getvalue"):
            file_bytes = archivo_binario.getvalue()
        elif hasattr(archivo_binario, "read"):
            if hasattr(archivo_binario, "seek"):
                archivo_binario.seek(0)
            file_bytes = archivo_binario.read()
        else:
            file_bytes = archivo_binario

        # [DOCUMENTACIÓN] Modificación 2026-07-06: Se envolvió la subida con ACL 'public-read' en un try/except. Si el bucket tiene las ACL desactivadas (AccessControlListNotSupported), se realiza una subida de respaldo sin indicar la ACL. Usamos io.BytesIO para evitar que boto3 cierre el stream original y cause "I/O operation on closed file".
        try:
            stream = io.BytesIO(file_bytes) if isinstance(file_bytes, bytes) else archivo_binario
            ...
            if "AccessControlListNotSupported" in str(e) or "AccessControlListNotSupported" in e.response.get("Error", {}).get("Code", ""):
                print("⚠️ El bucket S3 no permite ACLs (AccessControlListNotSupported). Reintentando subida de respaldo sin ACL...")
                stream_backup = io.BytesIO(file_bytes) if isinstance(file_bytes, bytes) else archivo_binario
                s3_client.upload_fileobj(
                    stream_backup,
                    bucket_name,
                    nombre_archivo_completo
        ...
        # [DOCUMENTACIÓN] Se corrigió la generación de url_archivo para utilizar nombre_archivo_completo en lugar de nombre_archivo, de forma que incluya la ruta de carpetas (prefix) real en S3.
...
# import io
# file_bytes = obtener_archivo_en_binario("formato_prueba.step")
...
#     nombre_archivo="formato_prueba_2.step", 
