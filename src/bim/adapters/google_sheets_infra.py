...
def procesar_y_extraer_sheets_costos_infra(ctx : PipelineContextCostos):
    """
    Escribe los datos en las hojas correspondientes y extrae
    el resultado de la hoja CALCULOS.
    """
    ...
    name_sheet_region = get_name_sheet_costos_for_region(ctx.data_project.region)
    datos = ctx.data_project.aforo
    region = ctx.data_project.region
    data_form_costos = ctx.data_req.model_dump()
    try:
        sh = conectar_google_sheets(ruta_json, "COSTOS_INFRAESTRUCTURA_2")
        ...
        ws_calculos = sh.worksheet("COSTOS INFRAESTRUCTURA")
        ...
        print(f"✅ Proceso completado. {len(df_calculos)} filas extraídas de CALCULOS.")
        ctx.data_calculo_costos = df_calculos.to_dict(orient="records")
