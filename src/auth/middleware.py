...
async def verify_token(request: Request, call_next):
    """
    Middleware de prueba: no verifica tokens, permite pasar todo.
    Útil solo para tests o desarrollo.
    """
    # 🔹 Puedes excluir rutas públicas si quieres, igual que la versión real
    path = request.url.path
    if path in ["/api/v1/login", "/docs", "/openapi.json"]:
        return await call_next(request)
    
    # 🔹 Simplemente permitimos pasar, sin decodificar nada
    request.state.user_id = "test_user"  # opcional, para simular un usuario
    return await call_next(request)